...
int maxw (int size1, int x_offset, int size2){
  if (size1 > x_offset + size2) {
    return size1;
  }
  else {
    return x_offset + size2;
  }
}

int maxh (int size1, int y_offset, int size2){
  if (size1 > y_offset + size2) {
    return size1;
  }
  else {
    return y_offset + size2;
  }
}
...
void squares(int size1, int x_offset, int y_offset, int size2)
{
  ...
  int w = maxw(size1, x_offset, size2);
  //compute the max of size1 and (y_offset + size2).  Call this h
  int h = maxh(size1, y_offset, size2);
  //TESTING_REMOVE printf ("%d \n", w);
  //TESTING_REMOVE printf ("%d \n", h);
